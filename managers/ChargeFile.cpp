...
ChargeFile::ChargeFile() {
	fatherIndex = new BPlusTree<SecondaryIndexRecord<ChargeRecord::Key, ChargeRecord::Key> , 4096> (
			CHARGE_FILE_FATHER_INDEX_PATH);
	districtIndex = new BPlusTree<SecondaryIndexRecord<DistrictRecord::Key, ChargeRecord::Key> , 4096> (
			CHARGE_FILE_DISTRICT_INDEX_PATH);
...
void ChargeFile::insert(const ChargeRecord & record) {
	...
	if (record.hasFather()) {
		newFather = new SecondaryIndexRecord<ChargeRecord::Key, ChargeRecord::Key> (record.getChargeFather(),
				record.getKey());
		fatherIndex->insert(*newFather);
	}
	newDistrict = new SecondaryIndexRecord<DistrictRecord::Key, ChargeRecord::Key> (record.getDistrict(),
			record.getKey());
...
void ChargeFile::remove(const ChargeRecord & record) {
	...
	if (deletedRecord->hasFather()) {
		deletedFather = new SecondaryIndexRecord<ChargeRecord::Key, ChargeRecord::Key> (
				deletedRecord->getChargeFather(), deletedRecord->getKey());
		fatherIndex->remove(*deletedFather);
	}
	deletedDistrict = new SecondaryIndexRecord<DistrictRecord::Key, ChargeRecord::Key> (deletedRecord->getDistrict(),
...
void ChargeFile::update(const ChargeRecord & record) {
	...
	if (oldRecord->hasFather()) {
		oldFather = new SecondaryIndexRecord<ChargeRecord::Key, ChargeRecord::Key> (oldRecord->getChargeFather(),
				oldRecord->getKey());
	...
	oldDistrict = new SecondaryIndexRecord<DistrictRecord::Key, ChargeRecord::Key> (oldRecord->getDistrict(),
			oldRecord->getKey());
	newDistrict = new SecondaryIndexRecord<DistrictRecord::Key, ChargeRecord::Key> (record.getDistrict(),
			record.getKey());
