...
void ResultScreen::draw() {
	...
	std::cout << "Elija el criterio por el cual quiere ver el resultado y apriete ENTER" << std::endl;
	std::cout << "" << std::endl;
...
char ResultScreen::resultByDistrict() {
	...
	while (true) {
		if (record.getElection().getDate() == fecha) {
			std::cout << "Lista: " << record.getList().getName() << ", Cargo: "
					<< record.getElection().getCharge().getCharge() << " de "
					<< record.getElection().getCharge().getDistrict().getString() << ", Votos: " << record.getCount()
					<< std::endl;
			showedResults++;
		}
		if (showedResults == 5) {
...
char ResultScreen::resultByList() {
	...
	while (true) {
		if (showedResults == 0) {
			...
			std::cout << "Eleccion: Fecha: " << fecha << " " << cargo << " de " << distrito << endl;
		}
		std::cout << " Votos en " << record.getDistrict().getString() << " : " << record.getCount() << std::endl
				<< endl;
		showedResults++;
		if (showedResults == 5) {
			showedResults = 0;
...
char ResultScreen::resultByElection() {
	...
	std::string distElec;
	system("clear");
	std::cout << "Ingrese la fecha de la Eleccion a consultar con el siguiente formato YYYYMMDD y presione ENTER"
			<< std::endl;
	std::cout << "" << std::endl;
	bool fechaIncorrecta = true;
	while (fechaIncorrecta) {
	...
	std::cout << "" << std::endl;
	std::cout << "Ingrese el distrito del cual se mostraran los resultados y presione ENTER" << std::endl;
	std::cout << "" << std::endl;
	distrito = IstreamUtils::getString();
	std::cout << "" << std::endl;
	...
	try {
		...
		std::cout << "No se encuentran resultados para la Eleccion ingresada" << std::endl;
		std::cout << "" << std::endl;
		std::cout << "Quiere realizar otra busqueda S/N" << std::endl;
		return doQuestion();
	}
	...
	while (true) {
		try {
			...
			noResults = false;
			if (showedResults == 0)
				std::cout << "Resultados de eleccion: " << cargo << " de " << distElec << " en " << distrito << endl;

			std::cout<<"Lista: "<<record.getList().getName()<<"    "<<"Votos: "<<record.getCount()<<endl;
			record = vFile->nextElection();
